...
import { forApprovalSchema } from "./data/schema"

interface DataTableRowActionsProps<TData extends { contactNo?: string; address?: string }> {
  row: Row<TData & { createdAt: Date; updatedAt: Date | null; approverRemarks: string | null  }>;
}
...
export function DataTableRowActions<TData extends { contactNo?: string; address?: string }>({
  ...
  const task = forApprovalSchema.parse(originalData);
